...
BOOLEAN _IsAddressSafe(IN PVOID pCheckAddress)
{
    ...
    if (!((__readcr8() <= DISPATCH_LEVEL) ? g_pTheiaCtx->pMmIsAddressValid(pCheckAddress) : g_pTheiaCtx->pMmIsNonPagedSystemAddressValid(pCheckAddress)))
    {
        ...
        goto DIE_CALL_ERROR_IS_SAFE_ADDRESS;
    }
 
    do
    {
        if ((pCheckAddress >= pCurrentNode->DllBase) && (pCheckAddress <= (((PUCHAR)pCurrentNode->DllBase) + pCurrentNode->SizeOfImage))) { return TRUE; }
        else { pCurrentNode = pCurrentNode->InLoadOrderLinks.Flink; continue; }
    } 
    while (pCurrentNode != PsLoadedModuleList);

    if (g_VolatileNullByte) { DIE_CALL_ERROR_IS_SAFE_ADDRESS: DieDispatchIntrnlError(ERROR_IS_SAFE_ADDRESS); }
...
{  
    ...
    if (pModuleName)
    {
        ...
        do
        {
            if (g_VolatileNullByte) { continue0: continue; }
            ...
            if (!(FlagsExecute & SPII_GET_BASE_MODULE)) { goto NoReturnBaseAddress; }
...
NoReturnBaseAddress:
    ...
    {
        if (g_VolatileNullByte) { continue1: continue; }
        ...
        for (ULONG32 j = 0UI32; ((PUCHAR)pNameSection)[j] != 0x00UI8; ++j)
        {
            if ((pCurrentSectionHeader->Name)[j] != ((PUCHAR)pNameSection)[j])
            {
                ...
                {
                    DbgLog("[TheiaPg <->] _SearchPatternInImg: ection %s not found\n", ((PUCHAR)pNameSection));
    ...
    {
        if (g_VolatileNullByte) { continue2: continue; }
...
PVOID _SearchPatternInRegion(IN ULONG64 OptionalData[SPIR_AMOUNT_OPTIONAL_OBJS], IN ULONG32 FlagsExecute, IN PUCHAR pRegionSearch, IN PUCHAR pSig, IN PUCHAR pMaskSig, IN PUCHAR pStopSig, IN ULONG32 LenStopSig)
{
    ...
    while (TRUE)
    {
        if (g_VolatileNullByte) { continue0: ++pRegionSearch; continue; }
