...
static PVOID BuilderStubApcRoutine(IN PVOID pRoutine)
{
    ...
    if (!pPageStub)
    {
        ...
        goto DIE_CALL_ERROR_BUILD_STUB_APC_ROUTINE;
    }
    ...
    *(PVOID*)((PUCHAR)&CoreStubCall + 24) = pRoutine;
    ...
    if (g_VolatileNullByte) { DIE_CALL_ERROR_BUILD_STUB_APC_ROUTINE: DieDispatchIntrnlError(ERROR_BUILD_STUB_APC_ROUTINE); }
...
VOID InitSearchPgSysThread(VOID)
{
    ...
    if (__readcr8() > DISPATCH_LEVEL)
    {
        ...
        goto DIE_CALL_ERROR_INIT_SEARCH_PG_SYS_THREAD;
    }
    ...
    if (g_VolatileNullByte) { DIE_CALL_ERROR_INIT_SEARCH_PG_SYS_THREAD: DieDispatchIntrnlError(ERROR_INIT_SEARCH_PG_SYS_THREAD); }
